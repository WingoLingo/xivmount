...
func ScrapeMember(fcid string) (FCData) {
	...
	// Scrape FC info
	fc.OnHTML(".entry__freecompany__box", func(e *colly.HTMLElement) {
		if page <2 {
...
//			fmt.Println(freeData.FreeCompanyName, freeData.FreeCompanyWorld)
	...
	var totalPages int
	fc.OnHTML(".btn__pager", func(e *colly.HTMLElement) {
	...
	// Scrape member list
	fc.OnHTML("a.entry__bg", func(e *colly.HTMLElement) {
		member := e.Attr("href")
		member = reMember.ReplaceAllString(member, "")
	...
	fc.Visit(fmt.Sprintf("https://na.finalfantasyxiv.com/lodestone/freecompany/%s/member/?page=1", fcid))
